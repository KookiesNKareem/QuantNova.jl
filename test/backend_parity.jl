...
@testset "Backend Parity" begin
    ...
    fd_grad = QuantNova.gradient(f, x; backend=ForwardDiffBackend())
    fd_val, fd_grad2 = QuantNova.value_and_gradient(f, x; backend=ForwardDiffBackend())

    @testset "PureJulia matches ForwardDiff" begin
        pj_grad = QuantNova.gradient(f, x; backend=PureJuliaBackend())
        @test pj_grad ≈ fd_grad atol=1e-6

        pj_val, pj_grad2 = QuantNova.value_and_gradient(f, x; backend=PureJuliaBackend())
        @test pj_val ≈ fd_val
