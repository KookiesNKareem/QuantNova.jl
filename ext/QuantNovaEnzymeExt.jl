module QuantNovaEnzymeExt

using QuantNova
using QuantNova.AD: EnzymeBackend, _gradient, _hessian, _jacobian, _value_and_gradient
...
function QuantNova.AD._gradient(b::EnzymeBackend, f, x)
    if b.mode == :reverse
        dx = zero(x)
...
function QuantNova.AD._hessian(::EnzymeBackend, f, x)
    n = length(x)
...
function QuantNova.AD._jacobian(::EnzymeBackend, f, x)
    y0 = f(x)
    m = length(y0)
    n = length(x)
...
function QuantNova.AD._value_and_gradient(::EnzymeBackend, f, x)
    dx = zero(x)
